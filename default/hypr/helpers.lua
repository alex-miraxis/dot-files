...
local function command_from(value, description)
  ...
  if value.archy then
    return "archy-launch-" .. value.archy
  ...
  elseif value.tui then
    if value.focus then
      return "archy-launch-or-focus-tui " .. shell_quote(value.tui)
    else
      return "archy-launch-tui " .. shell_quote(value.tui)
    end
  end
...
function o.launch_webapp(url)
  return "archy-launch-webapp " .. shell_quote(url)
end

function o.launch_webapp_sole(name, url)
  return "archy-launch-or-focus-webapp " .. shell_quote(name) .. " " .. shell_quote(url)
end

function o.launch_sole(match, command)
  return "archy-launch-or-focus " .. shell_quote(match) .. " " .. shell_quote(o.launch(command))
end

function o.bind_menu(keys, description, menu, options)
  o.bind(keys, description, menu and ("archy-menu " .. menu) or "archy-menu", options)
end

function o.bind_toggle(keys, description, toggle, options)
  o.bind(keys, description, "archy-toggle-" .. toggle, options)
end
