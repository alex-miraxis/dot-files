--
-- Dynamic Arch Unlocks Menu for Elephant/Walker
--
-- A "Default" entry restores the archy-shipped Plymouth via
-- archy-plymouth-reset. After that, every theme that has a preview-unlock.png
-- appears as a customised unlock; picking one runs archy-plymouth-set-by-theme
-- <theme>. Both run in a floating terminal so sudo can prompt.
--
Name = "archyunlocks"
NamePretty = "Arch Unlocks"
...
function GetEntries()
  ...
  local user_themes_dir = home .. "/.config/archy/themes"
  local archy_path = os.getenv("ARCHY_PATH") or ""
  local default_themes_dir = archy_path .. "/themes"
  local default_preview = archy_path .. "/default/plymouth/preview-unlock.png"
      ...
      if theme_name and not seen_themes[theme_name] then
        ...
        if file_exists(preview_path) then
          ...
          table.insert(entries, {
            ...
            PreviewType = "file",
            Actions = {
              activate = "archy-launch-floating-terminal-with-presentation "
                .. shell_escape("archy-plymouth-set-by-theme " .. shell_escape(theme_name)),
            },
          })
        end
      end
  ...
  local default_entry = {
    ...
    Actions = {
      activate = "archy-launch-floating-terminal-with-presentation "
        .. shell_escape("archy-plymouth-reset"),
    },
  }
