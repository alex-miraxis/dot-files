Name = "archyBackgroundSelector"
NamePretty = "Arch Background Selector"
Cache = false
...
function GetEntries()
  ...
  local theme_name_file = io.open(home .. "/.config/archy/current/theme.name", "r")
  ...
  local dirs = {
    home .. "/.config/archy/current/theme/backgrounds",
  }
  if theme_name then
    table.insert(dirs, home .. "/.config/archy/backgrounds/" .. theme_name)
  end
      ...
      for background in handle:lines() do
        ...
        if filename and not seen[filename] then
          ...
          table.insert(entries, {
            ...
            Value = background,
            Actions = {
              activate = "archy-theme-bg-set " .. ShellEscape(background),
            },
            Preview = background,
            PreviewType = "file",
          })
