...
class TranscodeAction(GObject.GObject, Nautilus.MenuProvider):
    def _launch_transcode(self, paths):
        wrapper = shutil.which("archy-launch-floating-terminal-with-presentation")
        binary = shutil.which("archy-transcode")
    ...
    def _make_item(self, paths):
        ...
        item = Nautilus.MenuItem(
            name="ArchTranscodeNautilus::transcode",
            label=label,
    ...
    def _tools_available(self):
        return bool(
            shutil.which("archy-launch-floating-terminal-with-presentation")
            and shutil.which("archy-transcode")
        )
